...
| 18 | Admin Dashboard — Sidebar, Map & KPIs | 11 | 9 | 82% |
| 19 | Admin — Alert Management | 6 | 0 | 0% |
| 20 | Admin — User Management | 5 | 0 | 0% |
| 21 | Admin — Pipeline Controls | 8 | 8 | 100% |
| 22 | Profile Management | 5 | 0 | 0% |
| 23 | Analytics | 9 | 0 | 0% |
| 24 | Notification Module | 5 | 0 | 0% |
| 25 | AI Assistant Chatbot | 5 | 0 | 0% |
| 26 | Shadcn UI + Unified Table Pattern | 6 | 6 | 100% |
| | **TOTAL** | **219** | **171** | **78.1%** |
...
## Phase 18 — Admin Dashboard: Sidebar, Map & KPIs
> **Goal:** The admin dashboard becomes a true operational view — a collapsible icon sidebar (Vercel/Linear inspired), a live Nigeria map with Mapbox markers coloured by risk level, four KPI cards summarising system state, and a polished state detail modal.
> **Prerequisite:** Phase 15 complete (admin panel features live).

| # | Task | Status |
|---|------|:------:|
| 1 | Rewrite `Layout.tsx` — collapsible sidebar with Lucide icons (LayoutGrid/Users/Activity/ClipboardList/Timer), icon-only mode at 60px width, tooltips on hover when collapsed, collapse state persisted to localStorage | [x] |
| 2 | Top bar inside main layout — sticky 56px bar with page title (derived from route) + notification bell + admin avatar | [x] |
| 3 | Backend: `GET /api/admin/dashboard/stats` — dedicated stats endpoint (deferred; KPIs computed client-side from existing endpoints) | [ ] |
| 4 | Install Mapbox GL JS in `frontend-admin` | [x] |
| 5 | Create `AdminMap` component — Nigeria map with Mapbox markers at each state's lat/lng, coloured by risk level (LOW green / MODERATE yellow / HIGH orange / CRITICAL red), scale on hover, popup on hover | [x] |
| 6 | Integrate `AdminMap` into Dashboard page — map as the primary view below KPI cards, state click opens detail modal | [x] |
| 7 | State click on map or table row → open state detail modal — full assessment scores, AI advisory tab switcher (EN/HA/YO/IG), last assessed timestamp | [x] |
| 8 | Manual trigger button inside state detail modal — calls `POST /api/admin/trigger/{state_id}`, shows loading state and result message | [x] |
| 9 | Add 4 KPI stat cards — Critical states / High states / Active alerts / Last assessment run; computed from `getStates()` + `getActiveAlerts()` with skeleton loading state | [x] |
| 10 | Replace state card grid with a proper table — columns: State / Code / Risk Level / Last Assessed / View; sortable by risk order; hover highlight; dark mode support | [x] |
| 11 | Auto-refresh KPI cards every 60 seconds without full page reload | [ ] |

---

## Phase 19 — Admin: Alert Management
> **Goal:** Admins can create, view, and close active alerts per state. Alerts are currently auto-generated by the AI engine; this phase adds full manual control.
> **Prerequisite:** Phase 18 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Backend: `POST /api/admin/alerts` — create a new active alert (fields: state_id, title, description, risk_level); sets started_at to now | [ ] |
| 2 | Backend: `PUT /api/admin/alerts/{id}/close` — sets ended_at to now and is_active to false | [ ] |
| 3 | Backend: `GET /api/admin/alerts` — paginated list of all alerts (active + historical), cursor-based, filterable by state_id and is_active | [ ] |
| 4 | Frontend: Alert management page (`/alerts`) in admin sidebar — table showing state, title, risk level badge, started/ended timestamps, and status | [ ] |
| 5 | Frontend: Create alert form — state selector dropdown, title input, description textarea, risk level dropdown (MODERATE / HIGH / CRITICAL) | [ ] |
| 6 | Frontend: Close alert button on each active row — confirmation prompt before calling close endpoint | [ ] |

---

## Phase 20 — Admin: User Management
> **Goal:** Admins can see all registered public users, their subscription preferences, and deactivate accounts if needed.
> **Prerequisite:** Phase 19 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Backend: `GET /api/admin/users` — cursor-paginated list of public users (role = "user") with id, full_name, email, is_active, created_at, and subscription_count | [ ] |
| 2 | Backend: `GET /api/admin/users/{id}/subscriptions` — list of active subscriptions for a user with state name and notification preferences | [ ] |
| 3 | Backend: `PATCH /api/admin/users/{id}/deactivate` — sets is_active to false; deactivated users cannot log in | [ ] |
| 4 | Frontend: User management page (`/users`) in admin sidebar — table with name, email, joined date, subscription count, active status badge, and deactivate button | [ ] |
| 5 | Frontend: User row expand or modal — shows the user's subscribed states with their notify_moderate / notify_high / notify_critical preferences | [ ] |

---

## Phase 21 — Admin: Pipeline Controls
> **Goal:** Admins can manually trigger the data ingestion pipeline per source and see the last run status of each source without touching the server.
> **Prerequisite:** Phase 20 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Create `PipelineRun` SQLAlchemy model — fields: id, source (enum: open_meteo / nasa_power / noaa / nhfr / ncdc / who_afro), started_at, completed_at, status (running / success / error), error_message | [ ] |
| 2 | Alembic migration for `pipeline_runs` table | [ ] |
| 3 | Update each data pipeline module to write a `PipelineRun` record on start and update it on completion or failure | [ ] |
| 4 | Backend: `POST /api/admin/pipeline/trigger/{source}` — runs the specified source ingestion in a background thread; accepts "all" to run the full cycle | [ ] |
| 5 | Backend: `GET /api/admin/pipeline/status` — returns the most recent `PipelineRun` record per source with status and timestamps | [ ] |
| 6 | Frontend: Pipeline controls page (`/pipeline`) in admin sidebar — table of 6 sources with last run time, duration, status badge (green / red / spinning), and a trigger button per row; "Run All" button at top | [ ] |

---

## Phase 22 — Profile Management
> **Goal:** Logged-in users on both portals can view their account details, update their display name, and change their password securely.
> **Prerequisite:** Phase 21 complete (admin); Phase 13 complete (public — already met).

| # | Task | Status |
|---|------|:------:|
| 1 | Backend: `GET /api/me/profile` — returns id, email, full_name, role, created_at for the authenticated user | [ ] |
| 2 | Backend: `PUT /api/me/profile` — updates full_name only; email is immutable | [ ] |
| 3 | Backend: `PUT /api/me/password` — requires current_password and new_password (min 8 chars); verifies current password before updating | [ ] |
| 4 | Frontend (admin): Profile page (`/profile`) in sidebar — displays name, email, role badge; inline name edit; password change form with current + new + confirm fields | [ ] |
| 5 | Frontend (public): Profile page (`/me/profile`) — same as admin profile but without role field; linked from the Navbar user menu | [ ] |

---

## Phase 23 — Analytics
> **Goal:** Both the admin panel and the public site have data-driven analytics views — admins see operational system metrics, public users see Nigeria's climate-health picture at a glance.
> **Prerequisite:** Phase 22 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Install Recharts in `frontend-admin` and `frontend-public` | [ ] |
| 2 | Backend: `GET /api/admin/analytics/risk-distribution` — count of states currently at each risk level (LOW / MODERATE / HIGH / CRITICAL) | [ ] |
| 3 | Backend: `GET /api/admin/analytics/assessment-trends` — number of assessments run per day over the last 30 days | [ ] |
| 4 | Backend: `GET /api/admin/analytics/top-risk-states` — top 10 states ranked by current overall_score descending | [ ] |
| 5 | Backend: `GET /api/admin/analytics/subscriber-counts` — number of active subscriptions per state, sorted descending | [ ] |
| 6 | Backend: `GET /api/analytics/risk-overview` (public, no auth) — national summary: risk level distribution + most-at-risk region | [ ] |
| 7 | Backend: `GET /api/analytics/trends` (public, no auth) — states that improved vs. worsened in the last 7 days based on risk level changes | [ ] |
| 8 | Frontend (admin): Analytics page (`/analytics`) in sidebar — 4 charts: risk distribution pie chart, 30-day assessment trend line chart, top risk states horizontal bar chart, subscriber counts bar chart | [ ] |
| 9 | Frontend (public): Insights page (`/insights`) — national risk summary cards, improving vs. worsening states table, regional risk breakdown | [ ] |

---

## Phase 24 — Notification Module
> **Goal:** Public users see in-app notifications for alerts on their subscribed states. Admins see in-app notifications for new HIGH or CRITICAL risk level changes across the platform.
> **Prerequisite:** Phase 23 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Add `notifications_last_read_at` column to `users` table — Alembic migration | [ ] |
| 2 | Backend: `GET /api/me/notifications` — returns active alerts for the user's subscribed states, ordered by started_at desc, with unread count based on notifications_last_read_at | [ ] |
| 3 | Backend: `POST /api/me/notifications/mark-read` — sets notifications_last_read_at to now for the authenticated user | [ ] |
| 4 | Frontend (public): Notification bell icon in Navbar — shows unread badge count; clicking opens a dropdown list of alert cards per subscribed state; marks all read on open | [ ] |
| 5 | Frontend (admin): Notification bell in admin sidebar header — shows count of new HIGH/CRITICAL risk state changes since last read; dropdown shows recent changes with state name, from/to level, and timestamp | [ ] |

---

## Phase 25 — AI Assistant Chatbot
> **Goal:** A floating chat widget on the public site lets any visitor ask natural-language questions about climate-health risks in Nigeria. The AI answers with real-time risk data as context.
> **Prerequisite:** Phase 24 complete.

| # | Task | Status |
|---|------|:------:|
| 1 | Backend: `POST /api/chat` — accepts user_message and optional state_id; fetches current risk data for all states (or the specified state) as context; calls gpt-4o with a health-advisor system prompt; returns the AI response | [ ] |
| 2 | Backend: Rate-limit `/api/chat` to 20 requests per IP per hour using an in-memory counter — prevents runaway API cost from anonymous users | [ ] |
| 3 | Frontend (public): Floating chat button fixed to bottom-right of every page — opens a chat drawer/panel without navigating away | [ ] |
| 4 | Frontend (public): Chat drawer — scrollable message history, text input, send button, typing indicator while waiting for response | [ ] |
| 5 | Frontend (public): Suggested starter questions shown when chat is empty — e.g. "What is the current risk in Lagos?", "Which states should I avoid this week?", "What does a HIGH risk level mean?" | [ ] |
